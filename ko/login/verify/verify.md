...
네아로 애플리케이션 API 설정 시 ‘이용자 식별자(기본 제공 정보)’ 외의 정보(예: 이름, 메일 주소, 휴대전화번호, 생일, 성별, 연령대 등) 조회 권한을 선택했다면, 해당 정보가 서비스에서 활용되는 사례를 확인합니다(필수/추가 정보 구분 없이 ‘이용자 식별자’ 외에 조회 권한을 선택한 정보는 모두 활용 사례를 확인함).
...
네아로에서 전달하는 ‘이용자 식별자’는 애플리케이션마다 다르게 전달되기 때문에 이미 적용되었던 애플리케이션을 다른 애플리케이션으로 변경하는 경우 기존에 연동한 이용자를 식별하지 못하는 등의 문제가 발생합니다.
