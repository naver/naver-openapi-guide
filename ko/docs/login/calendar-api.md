---
title: 네이버 캘린더 일정 추가 API 명세
description: 네이버 사용자 캘린더에 일정을 추가할 수 있는 로그인 오픈 API의 명세와 호출 예제를 안내합니다.
---

# 캘린더 일정 추가 API 명세

네이버 사용자 캘린더에 일정을 추가할 수 있는 로그인 오픈 API입니다. 기존 REST API처럼 요청 URL과 요청 변수로 호출하는 방법은 동일하나, OAuth 2.0 인증 기반이므로 추가적으로 [네이버 로그인 API](/docs/login/api)를 통해 접근 토큰(access token)을 발급받아,
...
| :---: | :---: | :--- | :--- |
| <span class="ndc-color">403</span> | 1403 | 권한이 없습니다. / 해당 캘린더의 마스터, 관리자, 또는 정회원이 아닌 경우 |  |
| <span class="ndc-color">404</span> | 1404 | 존재하지 않는 캘린더입니다. / 캘린더가 삭제됐거나 캘린더 ID가 잘못된 경우 |  |
| <span class="ndc-color">500</span> | 1500 | 일시적인 서버 오류입니다. / 서버 오류가 반복되면 담당자에게 문의한다. | 서버 내부 에러가 발생하였습니다. 포럼에 올려주시면 신속히 조치하겠습니다. |
...
| 속성 | 설명 | 참고 링크 |
| :--- | :--- | :--- |
| UID | 일정의 고유한 ID 값<br> 하나의 캘린더 ID에서는 iCalendar의 UID가 고유해야 한다.<br> UID는 일반적으로 iCalendar를 최초 생성할 때 사용자 ID, 타임스탬프, 도메인 등의 정보를 조합해서 만든다. 이때 특수 기호 % 문자는 이스케이프 문제로 지원되지 않으니 주의한다. | [http://www.kanzaki.com/docs/ical/uid.html](http://www.kanzaki.com/docs/ical/uid.html) |
| DTSTART | 일정 시작 시간<br> 별도의 표준시간대를 사용하지 않는다면 ‘T’ 이하의 시간 값만 수정해서 사용할 수 있다. | [http://www.kanzaki.com/docs/ical/dtstart.html](http://www.kanzaki.com/docs/ical/dtstart.html) |
| DTEND | 일정 종료 시간<br> 별도의 표준시간대를 사용하지 않는다면 ‘T’ 이하의 시간 값만 수정해서 사용할 수 있다. | [http://www.kanzaki.com/docs/ical/dtend.html](http://www.kanzaki.com/docs/ical/dtend.html) |
| SUMMARY | 일정 제목 | [http://www.kanzaki.com/docs/ical/summary.html](http://www.kanzaki.com/docs/ical/summary.html) |
| DESCRIPTION | 일정의 상세 내용 | [http://www.kanzaki.com/docs/ical/description.html](http://www.kanzaki.com/docs/ical/description.html) |
| LOCATION | 장소 정보 | [http://www.kanzaki.com/docs/ical/location.html](http://www.kanzaki.com/docs/ical/location.html) |
| RRULE | 반복 정보 | [http://www.kanzaki.com/docs/ical/rrule.html](http://www.kanzaki.com/docs/ical/rrule.html)<br> 파싱용 라이브러리:<br>[http://jakubroztocil.github.io/rrule/](http://jakubroztocil.github.io/rrule/) |
| ORGANIZER | 약속 일정인 경우 일정의 마스터 정보<br> 약속 일정이 아니면 ORGANIZER를 빼고 iCalendar를 생성한다. | [http://www.kanzaki.com/docs/ical/organizer.html](http://www.kanzaki.com/docs/ical/organizer.html) |
| ATTENDEE | 약속 일정인 경우 참석자 정보<br> 약속 일정이 아니면 ORGANIZER를 빼고 iCalendar를 생성한다. | [http://www.kanzaki.com/docs/ical/attendee.html](http://www.kanzaki.com/docs/ical/attendee.html) |
