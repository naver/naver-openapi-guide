---
title: 용어 정리
description: 네이버 개발자센터에서 사용하는 오픈 API 관련 용어를 API의 기본, API 인증, API 호출, HTTP로 나누어 정확하게 정의합니다.
---
...
애플리케이션은 API를 활용해 만들어진 다양한 프로그램 결과물입니다. 네이버 개발자센터에서는 개발자가 네이버 오픈API를 활용해 웹 서비스, 모바일 웹 서비스, 모바일 앱 등을 의미합니다. 네이버 오픈API를 이용하려면 네이버 개발자센터의 **[애플리케이션 등록](/myapps/new)** 메뉴에서 애플리케이션 정보를 등록해야 합니다.
...
네이버 오픈API 사용자가 인증된 사용자인지를 확인하는 고유한 아이디와 비밀번호입니다. 네이버 개발자센터의 **애플리케이션 등록** 메뉴에서 [애플리케이션을 등록](/docs/openapiguide/appregister#애플리케이션-등록)하면 발급되는 값입니다.
...
웹 서버로 API 요청을 보내면 웹 서버에서는 API 호출이 정상적으로 되었는지에 대한 응답을 HTTP 상태 코드 값과 함께 반환합니다. 예를 들어 호출이 정상적으로 이뤄졌을 때는 상태 코드로 200을 반환합니다. API 호출 방법이 잘못되어 오류가 발생했을 때는 400, 403, 404 등을 상태 코드로 반환합니다. 서버에 오류가 있을 때는 500을 상태 코드로 반환합니다. 네이버 오픈API에서 사용하는 상태 코드는 "[오류 코드](/docs/openapiguide/errorcode#주요-오류-코드)"를 참고하십시오.
...
OAuth 2.0으로 인증에 성공하면 네이버 플랫폼에서는 접근 토큰(access token)이라는 복잡한 문자열을 애플리케이션에 전달합니다. 이 값은 [로그인 방식 오픈 API](/docs/openapiguide/apilist#로그인-방식-오픈-api)를 호출할 때 사용합니다. 네이버 오픈API 중에서 회원 프로필 조회, 카페 가입과 같은 기능을 제공하는 API는 회원 인증이 완료되어야만 API를 호출할 수 있는 API입니다. 이런 오픈 API를 호출할 때는 요청 헤더에 접근 토큰을 함께 보내야 합니다.
