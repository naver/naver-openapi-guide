...
- 애플리케이션 개발 상태: 로그인 방식 오픈 API를 사용하는 애플리케이션이 개발 중인지 서비스 적용 상태인지 확인하고 상태를 변경할 수 있습니다. 개발 상태가 '개발 중'이면 **멤버 관리** 탭에서 테스터 ID로 등록된 네이버 아이디로만 애플리케이션에 접속할 수 있습니다. 테스터 ID는 최대 20개까지만 등록할 수 있습니다. 개발이 완료되고 실제 서비스에 적용한 상태면 **서비스 적용**을 클릭해 상태를 변경하십시오. 개발 상태가 '서비스 적용'으로 변경되면 로그인 아이디 개수에 제한 없이 누구나 네이버 아이디로 애플리케이션에 접속할 수 있습니다.

<div class="danger">
	<p><strong>주의</strong></p>
	<p>애플리케이션 개발 상태를 허위로 변경하면 네이버 아이디로 로그인 서비스 이용이 정지될 수 있습니다. 반드시 개발이 완료된 서비스만 '서비스 적용' 상태로 변경하십시오.</p>
</div>  
